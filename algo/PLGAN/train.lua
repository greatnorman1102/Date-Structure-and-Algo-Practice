...
opt = {
   ...
   lr = 0.0002,            -- initial learning rate for adam 0.0002
    ...
    for i = 1, math.min(data:size(), opt.ntrain), opt.batchSize do
        ...
        if counter % opt.display_freq == 0 and opt.display then
            ...
            if opt.preprocess == 'colorization' then 
                local real_A_s = util.scaleBatch(real_A:float(),256,256)
                local fake_B_s = util.scaleBatch(fake_B:float(),256,256)
                local real_B_s = util.scaleBatch(real_B:float(),256,256)
                ...
                disp.image(util.deprocess_batch(util.scaleBatch(real_A:float(),256,256)), {win=opt.display_id, title=opt.name .. ' input'})
                disp.image(util.deprocess_batch(util.scaleBatch(fake_B:float(),256,256)), {win=opt.display_id+1, title=opt.name .. ' output'})
                disp.image(util.deprocess_batch(util.scaleBatch(real_B:float(),256,256)), {win=opt.display_id+2, title=opt.name .. ' target'})
            end
        end
