    ...
    def GenPatchInLoop(self, row_idx, col_idx, count):
        ...
        for patch_idx in range(int(start_patch), int(end_patch)):
			...
			if not os.path.exists(b_path):
    				os.makedirs(b_path)
			scipy.misc.imsave((b_path + '/b_patch_%d.jpg' % patch_idx), b_patch)
			
			d_path = os.path.join(self.map_path, 'd_patch_20m')
			if not os.path.exists(d_path):
    				os.makedirs(d_path)
			scipy.misc.imsave((d_path + '/d_patch_%d.jpg' % patch_idx),  d_patch)
			#print('Writing d_patch to %s' % d_path)

			c_path = os.path.join(self.map_path, 'c_patch_20m')
			if not os.path.exists(c_path):
    				os.makedirs(c_path)
			scipy.misc.imsave((c_path + '/c_patch_%d.jpg' % patch_idx),  b_patch+d_patch)
			#print('Writing d_patch to %s' % c_path)

                        data_tensor[idx] = patch
